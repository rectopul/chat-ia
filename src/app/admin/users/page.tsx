    ...
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-2">
            ...
            <p className="text-slate-500">Acompanhe e gerencie todos os usuários que interagiram com seus bots.</p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-white p-1 rounded-lg border border-slate-200 shadow-sm overflow-x-auto">
            ...
            {filters.map((f) => (
                <Link key={f.value} href={f.value ? `/admin/users?filter=${f.value}` : "/admin/users"}>
                    <Button
                        variant={filter === f.value || (!filter && !f.value) ? "default" : "ghost"}
                        size="sm"
        ...
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow className="hover:bg-transparent border-slate-100">
              <TableHead className="font-bold text-slate-700">ID / Nome</TableHead>
              <TableHead className="font-bold text-slate-700">Telegram</TableHead>
              <TableHead className="font-bold text-slate-700">Última Atividade</TableHead>
              <TableHead className="font-bold text-slate-700 text-center">Assinante</TableHead>
          ...
          <TableBody>
            {users.map((user) => (
              <TableRow key={user.id} className="border-slate-50 hover:bg-slate-50/30 transition-colors">
                ...
                <TableCell>
                  <div className="text-slate-700 font-medium">@{user.username || "n/a"}</div>
                ...
                <TableCell>
                  <div className="flex items-center gap-2 text-slate-600">
                    ...
                    <span className="text-sm">
                        {user.lastSeenAt.toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                </TableCell>
                <TableCell className="text-center">
                  {user.isSubscriber ? (
                    <Badge variant="default" className="bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-50 shadow-none">
