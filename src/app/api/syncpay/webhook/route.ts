  ...
  try {
    ...
    if (data.status === "paid" || data.status === "completed" || data.status === "succeeded") {
      ...
      const sale = await prisma.sale.findFirst({
        where: {
            OR: [
                ...
                { rawPayload: { path: ["pix_code"], equals: data.pix_code } } // Advanced query if needed
            ]
        },
      ...
      if (targetSale && targetSale.status !== SaleStatus.PAID) {
        ...
        if (targetSale.product.productType === "SUBSCRIPTION" && targetSale.product.subscriberDays) {
          const currentUntil = targetSale.user.subscriberUntil || new Date();
          const baseDate = currentUntil > new Date() ? currentUntil : new Date();
          newUntil = new Date(baseDate.getTime() + targetSale.product.subscriberDays * 24 * 60 * 60 * 1000);
        }
