...
export default async function AdminBotsPage() {
  ...
  async function createBot(formData: FormData) {
    ...
    const isUserAccount = formData.get("isUserAccount") === "true";
    ...
    if (isUserAccount) {
        ...
        await prisma.botAccount.create({
            data: { name, phoneNumber, apiId, apiHash, session, isUserAccount: true }
        });
    } else {
        const token = formData.get("token") as string;
        const webhookSecret = formData.get("webhookSecret") as string;
        await prisma.botAccount.create({
            data: { name, token, webhookSecret, isUserAccount: false },
        });
    }
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white p-6 rounded shadow-sm">
        <h3 className="text-lg font-semibold mb-4">Adicionar Novo Bot / User Account</h3>
        <form action={createBot} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input name="name" placeholder="Nome" className="p-2 border rounded" required />
          <select name="isUserAccount" className="p-2 border rounded">
              <option value="false">Bot API</option>
              <option value="true">User Account (TDLib/GramJS)</option>
          </select>
          <hr className="md:col-span-3" />
          <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                  <p className="text-xs font-bold">Para Bot API:</p>
                  <input name="token" placeholder="Bot Token" className="p-2 border rounded w-full" />
                  <input name="webhookSecret" placeholder="Webhook Secret" className="p-2 border rounded w-full" />
              </div>
              <div className="space-y-2">
                  <p className="text-xs font-bold">Para User Account:</p>
                  <input name="phoneNumber" placeholder="Telefone (+55...)" className="p-2 border rounded w-full" />
                  <input name="apiId" placeholder="API ID" className="p-2 border rounded w-full" />
                  <input name="apiHash" placeholder="API Hash" className="p-2 border rounded w-full" />
                  <textarea name="session" placeholder="Session String" className="p-2 border rounded w-full" rows={1} />
              </div>
          </div>
              ...
              <tr key={bot.id}>
                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900">
                  ...
                  <span className="text-xs text-gray-500 font-mono">
                    {bot.isUserAccount ? bot.phoneNumber : bot.token?.substring(0, 10)}...
                  </span>
                </td>
